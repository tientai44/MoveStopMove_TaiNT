...
public class WeaponMenu : UICanvas
{
    ...
    public void SetPageInformation(int page)
    {
        if (weaponShow != null)
        {
            GameObjectPools.GetInstance().ReturnToPool(GameObjectPools.GetInstance().weaponShows[weaponTypeShowing].ToString(), weaponShow);
        }
        weaponTypeShowing = weaponTypes[page];
        weaponName.text = weaponTypeShowing.ToString();
        weaponShow = GameObjectPools.GetInstance().GetFromPool(GameObjectPools.GetInstance().weaponShows[weaponTypeShowing].ToString(), weaponShowPos.position);
